...
public class JsonItemStack {
	...
	public static ItemStack fromJson(@Nullable String string) {
		...
		final JsonObject itemJson = Remain.GSON.fromJson(string, JsonObject.class);
		...
		if (lore != null)
			meta.setLore(lore.asList().stream().map(JsonElement::getAsString).collect(Collectors.toList()));
		...
		if (extraJson != null)
			if (meta instanceof SkullMeta) {
				...
				if (pages != null)
					bmeta.setPages(pages.asList().stream().map(JsonElement::getAsString).collect(Collectors.toList()));
